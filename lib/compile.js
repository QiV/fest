  ...
  var nsTags = 'doctype,comment,cdata,n,space,if,else,elseif,switch,case,default,value,insert,for,set,get,include'.split(',');
	...
	function _compile (data){
        ...
        case 'switch':
          ...
          break;
        case 'case':
          section.source.push('case {val}:'
            .replace('{val}', _getAttr(node, 'is', 'expr'))
          );
          return;
        case 'default':
          section.source.push('default:');
          return;
				case 'if':
          section.source.push([
            '(function(){',
        ...
        case 'elseif':
          ...
          return;
				case 'else':
					section.source.push('} else { return ""');
					return;
        case 'default':
          section.source.push('default:');
          return;
          ...
          section.source.push([
            'BLOCKS{name} = function(params) {',
              ...
              .replace('{name}', getName(_getAttr(node, 'name')))
					...
					return;
				case 'get':
          section.source.push('{return} BLOCKS[{block}]('
            .replace('{block}', node.attributes.select ?
                _getAttr(node, 'select', 'expr')
              : ('"' + _getAttr(node, 'name') + '"')
            ).replace('{return}', node.attributes.return ?
                'return' : ''
            )
          );
					return;
	...
	function finish_compile(file, options, name){
		...
		function build_template(output) {
          ...
          'var BLOCKS={},' +
            ...
            'debug_line="",' +
            'debug_block="",' +
            'htmlchars=/' + htmlchars.source + '/g,' +
            'htmlchars_test=/' + htmlchars.source + '/,' + // We need another regex because String.replace() won't change lastIndex to 0
            'short_tags = ' + JSON.stringify(short_tags) + ',' +
            'element_stack = [],' +
            'htmlhash=' + JSON.stringify(htmlhash) + ',' +
            'jschars=/' + jschars.source + '/g,' +
            'jschars_test=/' + jschars.source + '/,' +
            'jshash=' + JSON.stringify(jshash) +
          'function replaceHTML(chr){return htmlhash[chr]}' +
          'function replaceJS(chr){return jshash[chr]}' +
          'function escapeJS(s){' +
            'if (typeof s==="string") {' +
              'if (jschars_test.test(s))return s.replace(jschars,replaceJS);' +
            '} else if (typeof s==="undefined")return "";' +
            'return s;' +
          '}' +
          'function escapeHTML(s){' +
            'if (typeof s==="string") {' +
              'if (htmlchars_test.test(s))return s.replace(htmlchars,replaceHTML);' +
            '} else if (typeof s==="undefined")return "";' +
            'return s;'+
          '}' +
          source +
