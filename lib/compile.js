var compile = (function () {
  ...
  function _compile(data) {
    ...
    parser.onclosetag = function () {
      ...
      case 'get':
        ...
        section.source.push(
          '{return} BLOCKS[{name}]({getParams={},}{params}{,getParams})'
      ...
      case 'param':
        return;
      case 'var':
        var varSource = section.source.pop();
        section.source.push(varSource + ';"")');
        return;
      }
    };
  ...
  function finish_compile(file, options, name) {
    ...
    function build_template(output) {
      ...
      template = readFileSync(__dirname + '/compile_tmpl-js.js').toString()
        .replace('__NAME__', name)
        .replace(/__HTMLCHARS__/g, htmlchars.source)
        .replace('__SHORT_TAGS__', JSON.stringify(short_tags))
        .replace('__HTMLHASH__', JSON.stringify(htmlhash))
        .replace(/__JSCHARS__/g, jschars.source)
        .replace(/__JSHASH__/g, JSON.stringify(jshash))
