var compile = (function(){
	...
	var short_tags = {area: true, base: true, br: true, col: true, command: true,
						...
						link: true, meta: true, param: true, source: true, wbr: true, 'case': true , 'default': true, 'elseif': true, 'else': true };
	...
	function _compile (data){
		...
		parser.onopentag = function (node) {
			...
			switch (node.local){
				...
				case 'elseif':
					section.source += '} else if (#) {'.replace('#', [
						'(function(){',
							'try {',
								'__fest_if = ' + _getAttr(node, 'test', 'expr'),
							'}',
							'catch(e) {',
								'__fest_if = false;',
				...
				case 'case':
					if (siblings[siblings.length - 2] !== 'switch') 
						section.source += 'break;';
					
					section.source += 'case #:'.replace('#', _getAttr(node, 'is'));
					return;
