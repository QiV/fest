	...
	function _compile (data){
		...
		function closetag(name, opentag){
			...
			if (stack[stack.length - 1] === 'shorttag'){
				section.source += 'table.insert(html,"/>");';
			} else if (stack[stack.length - 1] === 'element') {
				section.source += 'fest_element=fest_element_stack[#fest_element_stack-1];';
				section.source += 'table.insert(html,fest_short_tags[fest_element]?"/>":">");';
			} else {
				section.source += 'table.insert(html,">");';
			}
			return false;
		...
		parser.onopentag = function (node) {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				for (var i = 0; i < attrs.expr.length; i++) {
					section.source += 'fest_attrs[' + i + ']=' + _getExpr(attrs.expr[i], 'expression #' + (i + 1) + ' in attribute "' + attrs.name[i] + '"');
				}
				stack.push('html:' + node.name);
				stack.push(node.name in short_tags ? 'shorttag' : 'tag');
				opentag = true;
				section.source += 'table.insert(html,"<' + node.name + attrs.text + '");';
				return;
			} else {
				fstack.push(node);
			}
			...
			switch (node.local){
				case 'element':
					...
					section.source += 'table.insert(html,"<" + __fest_element);';
					return;
				case 'template':
					...
					return;
				case 'doctype':
					section.source += 'table.insert(html,"<!DOCTYPE ");';
					return;
				case 'attribute':
					section.source += 'table.insert(html," ' + escapeJS(_getAttr(node, 'name')) + '=\\"");';
					return;
				case 'comment':
					section.source += 'table.insert(html,"<!--");';
					return;
				case 'cdata':
					section.source += 'table.insert(html,"<![CDATA[");';
					return;
				case 'text':
					section.source += node.attributes.value ? 'table.insert(html,"' + escapeJS(_getAttr(node, 'value')) + '");' : '';
					return;

				case 'lf':
				case 'space':
					section.source += 'table.insert(html,"'+(node.local == 'lf' ? '\\n' : ' ')+'");';
					return;
				...
				case 'value':
					...
					if (node.__fest_output === 'text'){
						...
						section.source += 'table.insert(html,__fest_escapeJS(';
					} else if(node.__fest_output === 'js') {
						section.source += 'table.insert(html,__fest_escapeJS(';
					} else if(node.__fest_output === 'json') {
						output.json = true;
						section.source += 'table.insert(html,__fest_escapeJSON(';
					} else{
						section.source += 'table.insert(html,__fest_escapeHTML(';
					}
					return;
				case 'script':
					if (!node.attributes.safe){
					...
					return;
				case 'call':
					section.source += 'try{' + 'table.insert(html,' + _getAttr(node, 'name', 'expr') + '(' + _getAttr(node, 'data', 'expr') + '))}catch(e){__fest_log_error(e.message)}';
					return;
				case 'insert':
					section.source += 'table.insert(html,"' + escapeJS(readFileSync(resolveFilename(_getAttr(node, 'src')), 'utf-8')) + '");';
					return;
				case 'each':
		...
		parser.onclosetag = function () {
			...
			if (node.ns[node.prefix] != fest_ns){
				...
				if (!(node.name in short_tags)){
					section.source += 'table.insert(html,"</' + node.name + '>");';
				}
				return;
			} else {
				fstack.pop();
			...
			switch (node.local){
				case 'element':
					...
					section.source += 'if(!(__fest_element in __fest_short_tags)){table.insert(html,"</" + __fest_element + ">")}';
				...
				case 'template':
					...
					return;
				case 'plural':
					section.source += 'table.insert(html,__fest_format(__fest_params[__fest_plural(__fest_select)], __fest_select));';
					return;
				case 'var':
					if (!node.attributes.select) {
						section.source += 'return __fest_buf;';
		...
		parser.ontext = parser.oncdata = function (text) {
			...
			if (stack[stack.length - 1] === 'script') {
				...
				section.source += 'table.insert(html,"' + escapeJS(text) + '");';
			}
		};
