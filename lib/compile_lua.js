    ...
    parser.onopentag = function (node) {
      ...
      if (node.ns[node.prefix] != fest_ns){
        ...
        /*for (var i = 0; i < attrs.expr.length; i++) {
          section.source += '\nfest_attrs[' + i + ']=' + _getExpr(attrs.expr[i], 'expression #' + (i + 1) + ' in attribute "' + attrs.name[i] + '"');
        }*/
        stack.push('html:' + node.name);
        stack.push(node.name in short_tags ? 'shorttag' : 'tag');
        opentag = true;
        //section.source += '\ntable.insert(html,[=[<' + node.name + attrs.text + ']=])\n';
        return;
      } else {
        fstack.push(node);
      }
      ...
      switch (node.local){
        case 'element':
          ...
          section.source += [
            '',
            'fest_element=' + _getAttr(node, 'select', 'expr'),
            'table.insert(fest_element_stack, fest_element)',
            'table.insert(html, "<")',
            'table.insert(html, fest_element)',
          ].join('\n');
          return;
        case 'template':
          var context_name = (node.attributes.context_name ? _getAttr(node, 'context_name', 'var') : 'context');
        ...
        case 'doctype':
          ...
          return;
        case 'attribute':
          section.source += '\ntable.insert(html, [=[ '+escapeJS(_getAttr(node,'name')) + '="]=])\n';
          return;
        case 'comment':
          section.source += '\ntable.insert(html,[=[<!--]=])\n';
          return;
        ...
        case 'lf':
        case 'space':
          section.source += '\ntable.insert(html,[=['+(node.local == 'lf' ? '\\n' : ' ')+']=])\n';
          return;
        ...
        case 'script':
          ...
          return;
        case 'call':
          section.source += '\ntable.insert(html,' + _getAttr(node, 'name', 'expr') + '(' + _getAttr(node, 'data', 'expr') + '))';
          return;
        case 'insert':
      ...
      switch (node.local){
        case 'element':
          section.source += [
            '',
            'fest_element = fest_element_stack[#fest_element_stack - 1]',
            'if not fest_short_tags[fest_element] then',
            '  table.insert(html, "</" .. fest_element .. ">")',
            'end',
            'table.remove(fest_element_stack)',
          ].join('\n');
          return;
        case 'doctype':
          section.source += '\ntable.insert(html,[=[>]=])\n';
