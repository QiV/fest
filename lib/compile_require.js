  ...
  function _compile(data) {
    ...
    parser.onopentag = function (node) {
      ...
      case 'switch':
        ...
        break;
      case 'case':
        section.source.push(section.source.pop() + 'case {val}: return ""'
          .replace('{val}', _getAttr(node, 'is', 'expr'))
        );
        return;
      case 'default':
        section.source.push(section.source.pop() + 'default: return ""');
        return;
      case 'if':
        section.source.push([
          '(function(){',
          'if ({val}) {'.replace('{val}', _getAttr(node, 'test', 'expr')), (node.attributes.vars ? 'var ' + _getAttr(node, 'vars', 'expr') + ';' : ''),
          'return ""'
        ].join(''));
        return;
      case 'elseif':
        var prevIf = section.source.pop().slice(0, -9); //'}}()||"")'
        section.source.push(prevIf + [
          '} else if ({val}) {', (node.attributes.vars ? 'var ' + _getAttr(node, 'vars', 'expr') + ';' : ''),
          'return ""'
      ...
      case 'else':
        ...
        if (node.attributes.vars) {
          vars = 'var ' + _getAttr(node, 'vars', 'expr') + ';';
        }
        section.source.push(prevIf + '} else { ' + vars + ' return ""');
        return;
      case 'value':
      ...
      case 'for':
        var forVal = [
          ...
          'var {value} = {list}[{i}]', (node.attributes.vars ? ',' + _getAttr(node, 'vars') : ''),
          ';',
          'buf += ""'
        ].join('')
        ...
        if (localVars) {
          localVars.push(forVal);
        } else {
          section.source.push(forVal);
        }
        return;
      case 'set':
        section.source.push([
      ...
      case 'var':
        ...
        section.vars.push(
          ...
          .replace('{value}', node.attributes.value ? _getAttr(node, 'value') : (node.attributes.select ? '(' + _getAttr(node, 'select', 'expr') + ')' : '""'))
        );
        return;
      case 'vars':
        var vars = [];
    ...
    parser.onclosetag = function () {
      ...
      case 'get':
        ...
        if (!onTextParamed) {
          section.vars.push('var getParams={};');
        }
    ...
    parser.ontext = function (text) {
      ...
      case 'script':
        section.vars.push(_getEval(text));
        //section.expressionBlock.push(_getEval(text));
        break;
      case 'get':
      case 'var':
      case 'value':
    ...
    parser.onend = function () {
      /*if (section.expressionBlock.length) {
        section.source[0] = section.expressionBlock.join(';') + section.source[0];
        section.expressionBlock = [];
      }*/
    };
    ...
    function build_template(output) {
      ...
      output.sections.forEach(function (section) {
        if (output.disable_sgo || section.name === undefined || output.uses[section.name]) {
          ...
          vars += section.vars.join(';');
        }
      });

      template = readFileSync(__dirname + '/tmpl-require.js').toString()
